...
#Transition frequency in KHz
w_0= 217105181895*1e3*1e-3
#w_1=400
w_1=0*100000
hbar=1.0545718e-34
...
rabi2p=0*dipol*Ip/hbar
rabi2m=0*dipol*Im/hbar
gamma=1e2
e_0=8.854187*1e-12
c=299792458
kb=1.38064852*1e-23
...
ss=np.logspace(0,2,200)
detp=(ss-1)*50*1e3
d=-detp[1:]
detm=d[::-1]
dettune=np.concatenate((detm,detp),axis=0)
detune=dettune[191:208]
#print(detune)
#number of velocity groups / length=101
log_arrv=np.logspace(0,1.0455,101)
vp=(log_arrv-1)*10
xv=-vp[1:]
vm=xv[::-1]
...
def velocity():
    ...
    while i in range (0,nv):
        #print(i)
        v=(2/3501)*np.sqrt(kb*T/m)*(3*(i+.5-nv/2))
        vel.append(v)
        i=i+1
...
def sat_spectrum():
    ...
    for k in range (0,nv):
        ...
        print("k",k,"result",w_0*vel[k]/c,"KHz")
